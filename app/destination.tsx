...
const DestinationScreen = () => {
  ...
  const [prevSelectedDestination, setPrevSelectedDestination] = useState<any>([]);
  const loadPrevSelectedCities = async()=> {
    try {
      const cities = await AsyncStorage.getItem('destinationCities');
      if(cities !== null){
        setPrevSelectedDestination(JSON.parse(cities))
      }
  ...
  const handleSelectAutoComplete = async(item: any)=>{
    const prevSelectedCities = [...prevSelectedDestination];
    prevSelectedCities.push({city: item.name, iataCode: item.iataCode});
    await AsyncStorage.setItem('destinationCities', JSON.stringify(prevSelectedCities));
    setPrevSelectedDestination(prevSelectedCities)
    setFlightOfferData({...flightOfferData, destinationLocationCode: item.iataCode});
             ...
             <View className="flex-row gap-4 justify-start items-center px-2">
                <Pressable
                    ...
                    <View className="w-[60%] justify-center items-center flex-row">
                      <Text className="text-lg text-white font-extrabold">Select Destination</Text>
                    </View>
                    <View>
                      <View>
        ...
        <View className="w-full py-4 px-4 relative">
          ...
          <View className="px-4 w-full">
            <Text className="text-gray-500 text-lg font-bold mt-4">Previouse Selected</Text>
              {prevSelectedDestination.map(({city, index})=>(
                <Pressable
                   key={index}
                   onPress={()=>{
                    setFlightOfferData({...flightOfferData, destinationLocationCode: city.iataCode});
